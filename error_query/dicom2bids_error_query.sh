...
$subject_files=$( grep -m 1 $subject *.err | cut -d: -f 1 ; ) ;
most_recent_file_info=$( ls -lrt $subject_files | head -n 1 ) ;
most_recent=$( basename $( echo $most_recent_file_info | awk '{print $NF}' ))
#echo $most_recent
time_limit_subjects=$( grep -l "DUE TO TIME LIMIT" $most_recent | cut -d_ -f2 | cut -d. -f1); #make if statement to grab run number if "DUE TO TIME LIMIT" is outputted
oom_subjects=$( grep -l "oom-kill" $most_recent | cut -d_ -f2 | cut -d. -f1 );
done
